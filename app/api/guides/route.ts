...
import { parseManualGemini } from "@/lib/parserGemini";
...
function chooseEngine(req: NextRequest): EngineChoice | null {
  ...
  if (browserKey) return { engine: "gemini", geminiKey: browserKey };
  if (process.env.GEMINI_API_KEY) {
    return { engine: "gemini", geminiKey: process.env.GEMINI_API_KEY };
  }
...
async function runPipeline(
  ...
  const guide =
    choice.engine === "gemini"
      ? await parseManualGemini(input, { apiKey: choice.geminiKey! })
