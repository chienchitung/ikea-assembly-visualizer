...
export function repairGuide(
  ...
  const g = obj(raw);
  const clampPage = (v: unknown, fallback: number) =>
    Math.min(Math.max(Math.round(num(v, fallback)), 1), Math.max(pageCount, 1));

  const product = obj(g.product);
  const parts = arr(g.parts).map((v, i) => {
    const p = obj(v);
  ...
  const tools = arr(g.tools).map((v, i) => {
    const t = obj(v);
    return {
      id: str(t.id, `T${i + 1}`),
  ...
  const warnings = arr(g.warnings).map((v, i) => {
    const w = obj(v);
    return {
      id: str(w.id, `W${i + 1}`),
  ...
  const steps = arr(g.steps).map((v, i) => {
    const s = obj(v);
