...
export async function parseManualGemini(
  input: ParseInput,
  { apiKey, model }: GeminiOptions
): Promise<AssemblyGuide> {
  const useModel = sanitizeModel(model) ?? DEFAULT_MODEL;
  ...
  let res = await callGemini(useModel, apiKey, body);
  ...
  if (res.status === 400) {
    ...
    if (/responseJsonSchema|response_json_schema|Unknown name/i.test(errText)) {
      ...
      res = await callGemini(useModel, apiKey, { ...body, generationConfig: rest });
    } else {
...
function geminiErrorMessage(status: number, raw: string): string {
  ...
  if (status === 404) return `找不到指定的 Gemini 模型,請在右上角改選其他模型。(${detail})`;
  return `Gemini API 錯誤(${status}):${detail}`;
}

/** 僅允許合法模型 id 字元,避免 header 內容拼進 URL */
export function sanitizeModel(model?: string | null): string | null {
  if (!model) return null;
  return /^[a-z0-9.-]{1,64}$/.test(model) ? model : null;
}
